...
@Component({
    selector: 'jhi-product-lpnm',
    templateUrl: './product-lpnm.component.html'
})
export class ProductLpnmComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/product-lpnm'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/product-lpnm', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProductLpnm) {
        return item.id;
    }
