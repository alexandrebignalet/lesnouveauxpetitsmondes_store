...
describe('Tag e2e test', () => {
    ...
    it('should load Tags', () => {
        navBarPage.goToEntity('tag-lpnm');
        tagComponentsPage = new TagComponentsPage();
        expect(tagComponentsPage.getTitle()).toMatch(/lesnouveauxpetitsmondesStoreApp.tag.home.title/);
    ...
    it('should load create Tag dialog', () => {
        ...
        expect(tagDialogPage.getModalTitle()).toMatch(/lesnouveauxpetitsmondesStoreApp.tag.home.createOrEditLabel/);
        tagDialogPage.close();
    });
...
export class TagComponentsPage {
    ...
    title = element.all(by.css('jhi-tag-lpnm div h2 span')).first();
